...
import { updateInstallmentAction, toggleInstallmentReceivedAction } from '@/app/actions/installments';
...
/** O carnê de uma venda ou prestação: dá baixa, corrige vencimento e cancela
 *  parcela a parcela. */
export function ParcelasList({ parcelas }: { parcelas: Installment[] }) {
  const [editando, setEditando] = useState<Installment | null>(null);
  ...
  return (
    <div className="rounded-control border border-border bg-card-dark p-4">
      ...
      <div className="grid grid-cols-[70px_110px_110px_110px_1fr_80px] gap-2 border-b border-divider pb-2 text-[10.5px] font-extrabold uppercase tracking-[.06em] text-fg-faded">
        <div>Parcela</div>
      ...
      {parcelas.map((p) => {
        ...
        return (
          <div
            key={p.id ?? p.number}
            className={`grid grid-cols-[70px_110px_110px_110px_1fr_80px] items-center gap-2 border-b border-divider py-2 text-[12.5px] last:border-b-0 ${
              p.status === 'Cancelada' ? 'opacity-55' : ''
            }`}
          >
            <div className="font-bold">{rotuloDaParcela(p, totalParcelas)}</div>
            ...
            <div className="flex justify-end gap-1.5">
              <button
                onClick={() => baixar(p)}
                disabled={pending || p.status === 'Cancelada'}
              ...
              <button
                ...
                className="grid h-6 w-6 place-items-center rounded-full border border-border-strong text-fg-tertiary hover:border-accent hover:text-accent disabled:opacity-50"
              >
                <Pencil size={11} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[60] grid place-items-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-[460px] rounded-[20px] border border-border-strong bg-card p-7">
            <div className="mb-1 text-[15px] font-extrabold">
              {editando.number === 0 ? 'Entrada' : `Parcela ${editando.number}`}
            </div>
            <div className="mb-5 text-[12.5px] text-fg-tertiary">{formatBRL(editando.amount)}</div>
